...
class App extends Component {
  ...
  fetchData() {
    fetch('/data.json').then(response => response.json()).then(data => {
      this.setState({
        excerpts: data.excerpts.map((item) => _.assign(item, {
  ...
  addExcerpt(newItem) {
    this.excerptID++;
    const excerpt = _.assign({}, newItem, {
      id: this.excerptID,
      favourite: false
    });
  ...
  addCategory(catName, titleName) {
    const _titleName = titleName !== null
      ? [titleName]
      : [];
    const newCategory = {
      name: catName,
  ...
  toggleCategories() {
    this.setState((prevState) => ({
      categoriesIsOpened: !prevState.categoriesIsOpened
    }));
  }

  toggleNewExcerpt() {
    const popups = mapToFalseValues(this.state.popups);
    this.setState((prevState) => ({popups: _.assign({}, popups, {
        newExcerptIsOpen: !prevState.popups.newExcerptIsOpen
      })}));
  }
  ...
  toggleNewCategory() {
    const popups = mapToFalseValues(this.state.popups);
    this.setState((prevState) => ({popups: _.assign({}, popups, {
        newCategoryIsOpen: !prevState.popups.newCategoryIsOpen
      })}));
  }

  closeAllPopups() {
    this.setState((prevState) => ({
      popups: mapToFalseValues(this.state.popups)
    }));
  }
  ...
  editExcerpt(excerpt) {
    ...
    const newExcerpts = this.state.excerpts.map((item) => item.id === newExcerpt.id
      ? newExcerpt
      : item);
