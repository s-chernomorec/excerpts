...
export class Excerpt extends Component {
	...
	acceptEdit() {
		...
		if (prevText !== nextText) {
			const validationResults = validate([
				validateField(nextText, {min: 3, max: 800, name: 'Excerpt text'})
			]);
  ...
  render() {
		const manageExcerptStyle = { display: this.state.mouseOverExcerpt ? 'block' : 'none' };
		const classWhenFavourite = this.props.excerpt.favourite ? 'excerpt-is-favourite' : '';
    ...
    return (
      <div className="excerpt-wrapper">
        ...
        <div className={`excerpt-body ${classWhenFavourite}`} onMouseEnter={this.showManageButtons} onMouseLeave={this.hideManageButtons}>
        	...
        	<Validation condition={!this.state.validationIsPassed && this.state.editIsActive} messages={this.state.validationMessages} validationStyle={{ margin: '0 0 10px 0' }}/>
					{ this.state.editIsActive
						? <Textarea text={this.props.text} ref={(input) => { this._textarea = input; }} />
						: <span className="excerpt-text" onClick={this.edit}>{this.props.text}</span> 
					}
					<ManageExcerpt 
						manageIsActive={ this.state.editIsActive }
						excerptID={ this.props.excerpt.id }
						favouriteCls={ this.props.excerpt.favourite ? 'heart' : 'heart-o' }
						acceptEdit={ this.acceptEdit } 
						cancelEdit={ this.cancelEdit }
						edit={ this.edit }
						remove={ this.remove }
						addToFavourites={ (id) => this.props.addToFavourites(id) }
						manageExcerptStyle={manageExcerptStyle} 
					/>
        </div>
      </div>
    );
