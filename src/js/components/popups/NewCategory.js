...
class NewCategory extends Component {
  ...
  submit(e) {
    ...
    const newCategoryName = e.target.newCategoryName.value;
		...
		const validationResults = validate([
			validateField(newCategoryName, {min: 3, max: 12, name: 'Category name'}),
			checkForUniqueness(newCategoryName, this.props.categories, 'name') ? null : 'Category name should be unique'
		]);
  ...
  render() {
    let displayPopup = { display: this.props.newCategoryIsOpen ? 'block' : 'none' };
    ...
    return (
      <Popup headerTitle="New Category" frameStyle={displayPopup}>
        <form onSubmit={this.submit}>
          ...
          <TextInputFormRow name="newCategoryName" labelText="Category" placeholder="3-12 characters" inputStyle={{ marginRight: 0 }} />
