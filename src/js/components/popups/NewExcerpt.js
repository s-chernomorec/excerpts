...
class NewExcerpt extends Component {
  ...
  chooseNewCategory(e) {
    ...
    this.setState({
      ...
      newTitleIsChosen: newCategoryIsChosen
        ? true
        : false
    });
  }
  ...
  chooseNewTitle(e) {
    ...
    this.setState((prevState) => ({
      newTitleIsChosen: !prevState.newTitleIsChosen
    }));
  }
  ...
  chooseCategory(obj) {
    ...
    if (obj) {
      if (obj.target)
        chooseOrUpdateValue = obj.target.value;
      if (obj.name)
        chooseOrUpdateValue = obj.name;
      }

    this.props.categories.forEach((cat) => {
      if (cat.name === chooseOrUpdateValue)
        chosenCategory = cat;
      }
    );
  ...
  submit(e) {
    ...
    const validationResults = validate([
      !checkForUniqueness(category, this.props.categories, 'name') && this.state.newCategoryIsChosen
        ? 'Category name should be unique'
        : null,
      !checkForUniqueness(title, this.props.chosenCategory.titles) && this.state.newTitleIsChosen
        ? 'Title should be unique in same category'
        : null,
    ...
    if (this.props.categories.length === 0) {
      ...
      this.setState({
        ...
        this.chooseCategory({name: category, titles: [title]})
      });
  ...
  render() {
    let displayPopup = {
      display: this.props.newExcerptIsOpen
        ? 'block'
        : 'none'
    };
    ...
    return (
      <Popup headerTitle="New Excerpt" frameStyle={displayPopup}>
        <form onSubmit={this.submit}>
          ...
          <NewExcerptFormRow label="Category" inputName="categoryName" placeholder="3-12 characters" optionList={this.props.categories} newItemIsChosen={this.state.newCategoryIsChosen} chooseItem={(e) => this.chooseCategory(e)} chooseNewItem={(e) => this.chooseNewCategory(e)}/>

          <NewExcerptFormRow label="Title" inputName="titleName" placeholder="3-24 characters" optionList={this.props.chosenCategory.titles} newItemIsChosen={this.state.newTitleIsChosen} chooseItem={(e) => true} chooseNewItem={(e) => this.chooseNewTitle(e)} newBtnIsActive={!this.state.newCategoryIsChosen}/>
