...
class Sidebar extends Component {
  ...
  render() {
    const submenu = this.props.categories.map((category) => {
      return <li key={category.name} onClick={this.props.closeAllPopups}><NavLink to={"/category/" + category.name} activeClassName="submenu-item-active">{category.name}</NavLink></li>
    });
        ...
        <ul className="menu">
          <li>
						<Search 
							...
							getSearchQuery={ (query) => this.props.getSearchQuery(query) }
          ...
          <li>
              ...
              <ul className="submenu submenu-categories">
                { this.props.categoriesIsOpened ? submenu : '' }
              </ul>
          </li>
