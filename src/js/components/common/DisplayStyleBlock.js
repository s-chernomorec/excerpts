...
const DisplayStyleButton = props => {
  ...
  const activeClass = props.activeStyle === props.thisStyle
    ...
    ? <div className="excerpts-display-style-wrapper">
        ...
        <span className="display-excerpts-block">
          {props.excerptDisplayStyles.map((item, i) => {
            return (<DisplayStyleButton key={i} activeStyle={props.excerptDisplayStyleActive.key} thisStyle={item.key} changeDisplayStyle={(style) => props.changeDisplayStyle(style)} excerptDisplayStyles={props.excerptDisplayStyles}/>);
          })}
        </span>
      </div>
    : null
